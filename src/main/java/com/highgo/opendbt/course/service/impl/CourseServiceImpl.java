...
public class CourseServiceImpl extends ServiceImpl<CourseMapper, Course> implements CourseService {
  ...
  public Integer deleteCourse(int courseId, HttpServletRequest request) {
    ...
    //删除作业模板
    List<THomeworkModel> homeworkModels = homeworkModelService.list(new QueryWrapper<THomeworkModel>().eq("course_id", courseId));
    if (!homeworkModels.isEmpty()) {
      List<Integer> homeworkModelIds = homeworkModels.stream().map(THomeworkModel::getId).collect(Collectors.toList());
      homeworkModelService.removeByIds(homeworkModelIds);
      modelExerciseService.remove(new QueryWrapper<TModelExercise>().in("model_id", homeworkModelIds));
    }
