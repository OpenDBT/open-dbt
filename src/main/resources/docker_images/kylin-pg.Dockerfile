...
RUN yum install -y --nogpgcheck  perl-ExtUtils-Embed readline-devel zlib-devel pam-devel libxml2-devel libxslt-devel openldap-devel python-devel glibc-headers gcc gcc-c++ openssl-devel cmake make gettext gettext-devel   curl  \
 && yum clean all
