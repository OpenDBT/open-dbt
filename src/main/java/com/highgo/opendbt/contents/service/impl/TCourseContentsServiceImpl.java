...
//    }
  ...
  private void updateAttachments(HttpServletRequest request, TCourseContents courseContents, UserInfo loginUser) {
    ...
    if (courseContents.getAttachments() != null && !courseContents.getAttachments().isEmpty()) {
      ...
      //为空的所有的都设置为删除
      catalogueResourcesService.update(new UpdateWrapper<TCatalogueResources>().set("delete_flag", 1)
        .eq("course_id", courseContents.getCourseId())
        .eq("catalogue_id", courseContents.getCatalogueId()));
      //逻辑删除所有更新进度
      catalogueProcessService.update(new UpdateWrapper<TCatalogueProcess>().set("delete_flag", 1)
        .eq("delete_flag", 0)
        .eq("course_id", courseContents.getCourseId())
        .eq("catalogue_id", courseContents.getCatalogueId()));
    }
