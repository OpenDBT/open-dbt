...
import com.highgo.opendbt.progress.manage.AsyncSaveHomeWork;
...
public class THomeworkServiceImpl extends ServiceImpl<THomeworkMapper, THomework>
  ...
  public double saveHomeWork(HttpServletRequest request, SaveHomework homeWork) {
    ...
    // 写入答案.设置作业状态为提交
    ArrayList<CompletableFuture<String>> res = new ArrayList<>();
    homeworkInfos.forEach(item -> {
      CompletableFuture<String> future = asyncSaveHomeWork.syncSaveHomeWork(item, homeWork, loginUser, request, score, tStuHomeworkInfos);
      res.add(future);
    });
    //所有任务执行完才放行
