...
import com.highgo.opendbt.catalogue.service.impl.TCatalogueResourcesServiceImpl;
...
public class TResourcesServiceImpl extends ServiceImpl<TResourcesMapper, TResources> implements TResourcesService {
  ...
  @Override
  public boolean delResources(HttpServletRequest request, TResources resources) {
    // 获取用户信息
    UserInfo loginUser = Authentication.getCurrentUser(request);
    TResources tResources = this.getById(resources.getId());
    if (!resources.getCourseId().equals(tResources.getCourseId())) {
